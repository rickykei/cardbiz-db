...
const AW_counter = db.aw_counter;
...
exports.downloadStaffAWExcel =  (req, res) => {
  console.log("entered AW_counter.downloadStaffAWExcel");
  const populate=['staff_id'];
	...
	if (company_id == undefined || company_id =="" || uid=="" || uid == undefined) {
	  ...
	  if (company_id!="63142fd5b54bdbb18f556016")
	 {
	  ...
	  console.log(query);
	  AW_counter.find(query).populate(populate)
  .then((objs) => {
	...
	let profCnts = [];
		...
		let worksheet = workbook.addWorksheet("staffAWlog");
		...
		res.setHeader(
		  "Content-Disposition",
		  "attachment; filename=" + "staffAW.xlsx"
		);
