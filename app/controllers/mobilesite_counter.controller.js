...
const MobileSite_counter = db.mobilesite_counter;
...
exports.downloadStaffMobileSiteExcel =  (req, res) => {
  console.log("entered MobileSite_counter.downloadStaffLogExcel");
  const populate=['staff_id'];
	...
	if (company_id == undefined || company_id =="" || uid=="" || uid == undefined) {
	  ...
	  if (company_id!="63142fd5b54bdbb18f556016")
	 {
	  ...
	  console.log(query);
	  MobileSite_counter.find(query).populate(populate)
  .then((objs) => {
	...
	let profCnts = [];
		...
		let worksheet = workbook.addWorksheet("staffmobilesitelog");
		...
		res.setHeader(
		  "Content-Disposition",
		  "attachment; filename=" + "staffMobileSite.xlsx"
		);
